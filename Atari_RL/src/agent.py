...
def nature_cnn(observation_space, depths=(32, 64, 64), final_layer=512):
    ...
    with torch.no_grad():
        n_flatten = cnn(
            torch.as_tensor(
                observation_space.sample()[None].transpose(0, 3, 2, 1)
            ).float()
        ).shape[1]
...
class Network(nn.Module):
    def __init__(self, env, device):
        super().__init__()
        self.num_ations = env.action_space.n
        conv_net = nature_cnn(env.observation_space)
        self.net = nn.Sequential(conv_net, nn.Linear(512, self.num_ations))
        self.device = device
    ...
    def act(self, obses, epsilon):
        ...
        for i in range(len(actions)):
            ...
            if rnd_sample <= epsilon:
                actions[i] = random.randint(0, self.num_ations - 1)
...
make_env = lambda: Monitor(
    UnityToGymWrapper(UnityEnvironment(), allow_multiple_obs=False),
